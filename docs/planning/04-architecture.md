...
|---|---|---|---|
| `join` | client -> server | `sessionId`, `userName`, `roomName`, `role` | First message after connection. Creates room if it does not exist. |
...
| `update_role` | client -> server | `role` | Change own role: `"voter"` or `"observer"`. |
| `timer_set_duration` | client -> server | `duration` | Set timer duration in seconds. |
| `timer_start` | client -> server | *(empty)* | Start the timer countdown. |
| `timer_reset` | client -> server | *(empty)* | Reset the timer to idle state. |
| `leave` | client -> server | *(empty)* | Explicit room leave (from Settings menu). |
| `room_state` | server -> client | `roomId`, `roomName`, `createdBy`, `phase`, `participants`, `result`, `timer` | Full room snapshot. Sent once on join/reconnect. |
| `participant_joined` | server -> client | `sessionId`, `userName`, `status`, `role` | New participant entered the room. |
...
| `role_updated` | server -> client | `sessionId`, `role` | Participant changed their role. |
| `timer_updated` | server -> client | `duration`, `state`, `startedAt`, `remaining` | Timer state changed. Broadcast to all. |
...
The `role` field is optional. Valid values: `"voter"` (default) or `"observer"`. Observers see votes but do not participate in voting.
...
```json
{ "type": "update_role", "payload": { "role": "observer" } }
```

Changes the participant's role. `role` is `"voter"` or `"observer"`. Server broadcasts `role_updated` to all participants.

#### `timer_set_duration`

```json
{ "type": "timer_set_duration", "payload": { "duration": 300 } }
```

Sets the timer duration in seconds. Server broadcasts `timer_updated` with the new state.
...
Starts the timer countdown. Server records `startedAt` and broadcasts `timer_updated` with `state: "running"`.
...
Resets the timer to idle state. Server broadcasts `timer_updated` with `state: "idle"`.
...
Broadcast when a participant changes their role. `role` is `"voter"` or `"observer"`.
...
```json
{
  ...
  "payload": {
    "duration": 300,
    "state": "running",
    "startedAt": 1712582400000,
    "remaining": 245
  }
}
```

Broadcast when timer state changes. Fields:
- `duration`: Timer duration in seconds.
- `state`: `"idle"`, `"running"`, or `"expired"`.
- `startedAt`: Unix timestamp in milliseconds when the timer was started, or `null` when idle.
- `remaining`: Remaining seconds. Equals `duration` when idle.

Note: The server does not track timer expiration. The `expired` state transition happens on the client side based on `startedAt` and `duration`. This means clients may show expiration at slightly different times due to clock skew.
