...
Основные риски сосредоточены в трёх областях: (1) безопасность WebSocket-соединений — отключена проверка Origin, отсутствует rate-limiting на сообщения внутри соединения, нет валидации входных данных (roomID, sessionID); (2) доступность (a11y) — модальные окна без focus trap, отсутствие ARIA-атрибутов, глобальное подавление outline; (3) UX-пробелы — нет индикации проблем с соединением, нет возможности сменить имя, колода карт не зафиксирована на мобильных. Ни одна из этих проблем не является блокирующей для внутрикомандного использования, но при публичном деплое группа security-находок требует обязательного исправления.
...
### [72] Модальные окна не управляют фокусом и не блокируют скролл
- **Severity:** HIGH
- **Источники:** фронтенд, продукт, адвокат дьявола
